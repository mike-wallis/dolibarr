...
class ActionsSearchbox
{
    ...
    // input_sel  — CSS selector(s) for inputs to attach autocomplete to (string or array).
    ...
    private static $PAGE_TYPES = [
        'products' => [
            ...
            'input_sel'  => ['input[name="search_label"]', 'input[name="search_ref"]'],
            'fill_param' => 'search_ref',   // products navigate to card via r.url; fill_param is fallback
        ],
        'societe' => [
    ...
    public function printCommonFooter($parameters, &$object, &$action, $hookmanager)
    {
        ...
        // Build JS array literal of selectors (supports string or array in config)
        $sels = (array)$matched_cfg['input_sel'];
        $sels_js = implode(', ', array_map(function ($s) {
            return "'" . dol_escape_js($s) . "'";
        }, $sels));

        ob_start();
        echo '<link rel="stylesheet" href="' . $css_url . '">' . "\n";
        $this->outputJs($ajax_url, $type_j, $sels_js, $fill_param);
...
(function () {
    ...
    function initSB(input) {
        ...
        function showDropdown(results, total) {
            closeDropdown();
            ...
            if (total && total > results.length) {
                ...
                footer.textContent = 'Showing ' + results.length + ' of ' + total;
        ...
        function setActive(idx) {
            ...
            active = Math.max(-1, Math.min(idx, items.length - 1));
        ...
        input.addEventListener('input', function () {
            ...
            timer = setTimeout(function () {
                fetch(SB_AJAX + '?type=' + SB_TYPE + '&q=' + encodeURIComponent(q), {credentials: 'same-origin'})
                    ...
                    .then(function (d) { d.results ? showDropdown(d.results, d.total) : closeDropdown(); })
                    .catch(closeDropdown);
            }, 300);
        });
    ...
    SB_SELS.forEach(function (sel) {
        var input = document.querySelector(sel);
        if (input) initSB(input);
    });
}());
</script>
JS;
